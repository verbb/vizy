...
const provision = spawn('ddev', ['test', '--task=browser', `--browser-run=${runId}`], { cwd: root, stdio: 'inherit' });
let provisionExit;
...
try {
    ...
    const browser = spawn(path.join(root, 'node_modules/.bin/playwright'), ['test', '--config=playwright.cp.config.ts', ...process.argv.slice(2)], { cwd: root, stdio: 'inherit' });
